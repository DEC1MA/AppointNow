...
export class EventService {
  ...
  async cancelBusinessEvents(body: { userId: string; businessId: string }) {
    return await transactions.event.cancelBusinessEvents(
      body.userId,
      body.businessId
    );
  }
  ...
  async create(body: {
    ...
    return await transactions.event.create(
      body.userId,
      body.businessId,
      body.startTime
    );
  ...
  async businessEvents(body: { businessId: string }) {
    return await transactions.event.businessEvents(body.businessId);
  }
}
