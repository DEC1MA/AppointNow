...
let request = async (url, headers, body) => {
  ...
  try {
    ...
    const { data } = await axios.post(`http://localhost:10000/${url}`, body, {
      headers: authHeaders,
    });
    return data;
  } catch (error) {
...
class Api {
  ...
  user = {
    ...
    complete(cCode, firstName, lastName) {
      return new Promise((resolve) => {
        request("user/complete", {}, { cCode, firstName, lastName }).then(
          (res) => {
            if (res.status === "success") {
              token = res.data.session.token;
            }
  ...
  business = {
    create(name, about, location, phone, workingDays, workingHours, duration) {
      return request(
        "business/create",
        {},
        {
...
(async () => {
  ...
  let res0 = await api.user.connectTelegram(
    "1234567890987654321",
    "edward",
    "kasperian"
  );
  ...
  let res5 = await api.business.create(
    ...
    ["sat", "sun", "mon"],
    [
      { start: 123, duration: 100 },
      { start: 456, duration: 100 },
    ],
    10
  );
  console.log(res5);
